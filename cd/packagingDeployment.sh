...
#   1. To create subsequent versions & install from a developer workstation assuming alias for his/her Scratch org is MyScratchOrg, run
#   cd/packagingDeployment.sh package MyScratchOrg
#   2. Update PACKAGE_VERSION in this script as new stable version

#   In Feature / Develop / Package Branch
#   1. To install latest version from a developer workstation only (already defined in script below) to the scratch org, run
#   cd/packagingDeployment.sh install MyScratchOrg

#   In Package branch, 
#   2. Create a git tag with the version and approve UAT deployment to trigger the UAT pipeline

#   Before promoting to Production (promote package version)
#   TODO: When running in Prod CD, promote the package version

#   To install in Salesforce environment, using continuous deployment pipeline (QA, UAT, Prod environment)
...
BUILD_NAME="ECFMG.ES-Base-Code - CI"
PACKAGE_NAME="ES-Base_Code"
PACKAGE_VERSION="ES-Base_Code@0.1.0-2"
